...
<title>Pozorište: Overview</title>
...
Diagram</h2>
<p>A simpler way to think about jobs in contrast to observables is that job are closer to a Unix process. It has an argument (command line flags), receive inputs (STDIN and interupt signals), and output values (STDOUT) as well as diagnostic (STDERR). They can be plugged one into another (piping), and can be transformed, synchronized and scheduled (fork, exec, cron).</p>
...
<div class="line">});</div>
<div class="line">job.output.subscribe(x =&gt; console.log(x));</div>
</div><!-- fragment --><h1><a class="anchor" id="autotoc_md2237"></a>
Limitations</h1>
